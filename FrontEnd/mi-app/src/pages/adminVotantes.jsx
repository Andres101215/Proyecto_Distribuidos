...
const VOTANTES_URL = `${API_GATEWAY}/usuarios/auth/voters`;
const REGISTRO_URL = `${API_GATEWAY}/usuarios/auth/register`;
...
export default function AdminVotantes() {
  ...
  const fetchVotantes = async () => {
    try {
      const { data } = await axios.get(VOTANTES_URL);
      setVotantes(data);
    } catch (error) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editandoCodigo) {
        await axios.put(`${VOTANTES_URL}/${editandoCodigo}`, formData);
        setMensaje('Estudiante actualizado exitosamente');
      } else {
        await axios.post(REGISTRO_URL, formData);
      ...
      setMensaje('Error al guardar votante');
    }
  };

  const handleEditar = votante => {
    setFormData({
      codigoEstudiantil: votante.codigoEstudiantil,
  ...
  const handleEliminar = async codigoEstudiantil => {
    ...
    try {
      await axios.delete(`${VOTANTES_URL}/${codigoEstudiantil}`);
    ...
    <div className="flex min-h-screen relative">
      ...
      <div className="flex-1 p-8 overflow-y-auto bg-cover bg-center relative" style={{ backgroundImage: 'url("/fondo.jpg")' }}>
        ...
        <div className="relative z-10">
          ...
          {mensaje && <div className="bg-yellow-100 text-yellow-800 p-4 rounded mb-6">{mensaje}</div>}
                  ...
                  {votantes.map(v => (
                    <tr key={v._id} className="hover:bg-gray-100">
                      ...
                      <td className="px-4 py-2 border space-x-2">
                        <button onClick={() => handleEditar(v)} className="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded">
                          Editar
                        </button>
                        <button onClick={() => handleEliminar(v.codigoEstudiantil)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded">
                          Eliminar
                        </button>
                      </td>
                    </tr>
